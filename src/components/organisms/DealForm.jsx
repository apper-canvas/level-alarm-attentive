    ...
    <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
                <label className="block text-sm font-medium text-secondary-600">Associated Contact *
                                </label>
                <Select
            ...
            <div className="space-y-2">
                <label className="block text-sm font-medium text-secondary-600">Associated Company
                                </label>
                <Select
            ...
            <div className="space-y-2">
                <label className="block text-sm font-medium text-secondary-600">Deal Stage
                                </label>
                <Select
                    value={formData.stage}
                    onChange={e => handleChange("stage", e.target.value)}>
                    {DEAL_STAGES.map(stage => <option key={stage.value} value={stage.value}>
                        {stage.label}
                    </option>)}
                </Select>
            </div>
            ...
            <div className="space-y-2">
                <label className="block text-sm font-medium text-secondary-600">Deal Source
                                </label>
                <Select
                    value={formData.source}
                    onChange={e => handleChange("source", e.target.value)}>
                    {DEAL_SOURCES.map(source => <option key={source.value} value={source.value}>
                        {source.label}
                    </option>)}
                </Select>
            </div>
            <div className="space-y-2">
                <label className="block text-sm font-medium text-secondary-600">Priority
                                </label>
                <Select
                    value={formData.priority}
                    onChange={e => handleChange("priority", e.target.value)}>
                    {PRIORITIES.map(priority => <option key={priority.value} value={priority.value}>
                        {priority.label}
                    </option>)}
                </Select>
            </div>
            <div className="space-y-2">
                <label className="block text-sm font-medium text-secondary-600">Assigned To
                                </label>
                <Select
                    value={formData.assignedTo}
                    onChange={e => handleChange("assignedTo", e.target.value)}>
                    {DEAL_OWNERS.map(owner => <option key={owner} value={owner}>
                        {owner}
                    </option>)}
                </Select>
            </div>
        ...
        <div className="space-y-4">
            <div className="flex items-center justify-between">
                ...
                <Button
                    ...
                    <ApperIcon name="Plus" size={16} />Add Product
                                </Button>
            </div>
            {formData.products.length === 0 && <div className="text-center py-8 text-secondary-500">
            ...
            {formData.products.map((product, index) => <div
                key={index}
                ...
                <div className="space-y-1">
                    ...
                    <div
                        className="flex items-center px-4 py-3 bg-secondary-50 rounded-lg text-secondary-900 font-semibold">
            ...
            {formData.products.length > 0 && <div
                className="border-t border-secondary-200 pt-4 space-y-2 bg-secondary-50 p-4 rounded-lg">
                <div className="flex justify-between text-secondary-600">
                    <span>Subtotal:</span>
                    <span className="font-medium">{formatCurrency(subtotal)}</span>
                </div>
                {discount > 0 && <div className="flex justify-between text-secondary-600">
                    <span>Discount:</span>
                    <span className="font-medium">-{formatCurrency(discount)}</span>
                </div>}
                {tax > 0 && <div className="flex justify-between text-secondary-600">
                    <span>Tax:</span>
                    <span className="font-medium">{formatCurrency(tax)}</span>
                </div>}
                <div
                    className="flex justify-between text-lg font-bold text-secondary-900 border-t border-secondary-300 pt-2">
                    <span>Total Deal Value:</span>
                    <span>{formatCurrency(totalValue)}</span>
                </div>
                <p className="text-xs text-secondary-500 mt-2">This total will override the Deal Value/Amount field above
                                  </p>
            </div>}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
                <label className="block text-sm font-medium text-secondary-600">Description/Notes
                                </label>
                <textarea
                    rows="4"
        ...
        {/* Form Actions */}
        <div
            className="flex flex-col sm:flex-row gap-3 pt-6 border-t border-secondary-200">
            <Button
                type="button"
                variant="ghost"
                onClick={onClose}
                disabled={isSubmitting}
                className="order-2 sm:order-1">Cancel
                          </Button>
            <div className="flex gap-3 order-1 sm:order-2 sm:ml-auto">
                {!deal && <Button
                    ...
                    disabled={isSubmitting}
                    className="flex items-center gap-2">
                    <ApperIcon name="Plus" size={16} />Save & Add Another
                                  </Button>}
                <Button type="submit" disabled={isSubmitting} className="flex items-center gap-2">
                    {isSubmitting ? <>
                        <ApperIcon name="Loader2" size={16} className="animate-spin" />Saving...
                                        </> : <>
